...
from shop.customers.models import User, RegisterModel
...
def orders():
    ...
    user = RegisterModel.query.filter_by(email=session['email']).first()
    ...
    db.session.commit()  # Commit pour générer un ID d'ordre
    ...
    for product_id, item in cart.items():
        ...
        if product:
            ...
            db.session.add(order_item)  # Ajout à la session pour enregistrement
            total_price += item['quantity'] * product.price

    # Mise à jour du prix total et validation
    order.total_price = total_price
    db.session.commit()  # Commit ici pour enregistrer la commande et les items
