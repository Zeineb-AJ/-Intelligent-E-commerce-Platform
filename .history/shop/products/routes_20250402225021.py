...
def home():
    ...
    # Produits en promotion (seulement si discount > 0)
    promo_products = Product.query.filter(Product.discount > 0).order_by(Product.pub_date.desc()).limit(3).all()
    
    # Si pas de produits en promotion, on prend les nouveaux produits
    if not promo_products:
        promo_products = Product.query.order_by(Product.pub_date.desc()).limit(3).all()
    
    # Produits tendance (les plus vendus)
    trend_products = Product.query.order_by(Product.sales.desc()).limit(3).all()
    
    # Nouveaux produits (les plus récents)
    new_products = Product.query.order_by(Product.pub_date.desc()).limit(3).all()
    
    # Pagination pour les produits normaux
    ...
    return render_template('products/index.html',
                         ...
                         new_products=new_products,brands=get_all_brands(), categories=get_all_categories())    
