...
from .models import Brand, Category, Product,Order,OrderData
...
def confirm_order():
    ...
    form = OrderConfirmationForm()  # Votre nouveau formulaire
    
    if form.validate_on_submit():
        # Calcul du total
        total_price = sum(
            float(product['price']) * int(product['quantity'])
            for product in session['shopcart'].values()
        )
        ...
        analytics_data = OrderData(
            order_id=new_order.id,
            customer_id=current_user.id,
            gender=form.gender.data,
            age=form.age.data,
            product_category=form.product_category.data,
            quantity=sum(int(p['quantity']) for p in session['shopcart'].values()),
        ...
        if form.photo.data:
            ...
            photo_path = os.path.join(current_app.root_path, 'static/order_photos', photo_filename)
            form.photo.data.save(photo_path)
            analytics_data.photo_url = f'order_photos/{photo_filename}'

        db.session.commit()
        session.pop('shopcart', None)
        flash('Commande confirmée avec succès !', 'success')
        return redirect(url_for('order_success'))  # Nouvelle route pour la page de succès

    # Pré-remplissage pour GET request
    if request.method == 'GET':
        form.customer_id.data = current_user.id
        # Calcul automatique du prix moyen
        if session['shopcart']:
            total = sum(float(p['price']) for p in session['shopcart'].values())
            form.price_per_unit.data = round(total / len(session['shopcart']), 2)

    return render_template('orders/confirm.html',
                         form=form,
                         cart=session['shopcart'],
                         total=sum(float(p['price'])*int(p['quantity']) for p in session['shopcart'].values()))
def get_all_brands():
