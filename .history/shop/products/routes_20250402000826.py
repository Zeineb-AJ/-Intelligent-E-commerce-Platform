...
def home():
    ...
    products = Product.query.filter(Product.stock.isnot(None), Product.stock > 0).paginate(page=page, per_page=4)
    
    return render_template(
        'products/index.html', 
        pagination=products, 
        title="Store Home", 
        products=products.items,  # Fix: Pass only the items list
