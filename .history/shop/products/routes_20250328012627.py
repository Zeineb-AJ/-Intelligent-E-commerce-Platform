...
def confirm_order():
    ...
    form = OrderConfirmationForm(request.form)  # Ajout de request.form
    ...
    if form.validate_on_submit():
        try:
            ...
            # 2. Création des données de confirmation
            ...
            if not all([new_order.id, confirm_order.order_id, confirm_order.customer_id]):
                raise ValueError("Données requises manquantes")

            db.session.commit()  # Commit explicite
            
            # Nettoyage et redirection
            session.pop('shopcart', None)  # Plus sûr avec None par défaut
            ...
            return redirect(url_for('products.cart'))  # Redirection en cas d'erreur

    # Pré-remplissage pour GET
    if request.method == 'GET':
        form.gender.data = form.gender.data or 'M'  # Valeur par défaut
        if session['shopcart']:
