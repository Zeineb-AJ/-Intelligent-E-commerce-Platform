...
@login_required
def confirm_order():
    # Vérification initiale du panier
    if not session.get('shopcart'):
    ...
    if form.validate_on_submit():
        try:
            with db.session.begin_nested():  # Transaction imbriquée
                # Calculs
                cart_items = session['shopcart'].values()
                total_price = sum(
                    float(p['price']) * int(p['quantity'])
                    for p in cart_items
                )
                ...
                # Debug: Affiche les valeurs avant création
                app.logger.info(f"Création commande ID: {new_order.id}")
                app.logger.info(f"Données formulaire: {form.data}")

                # Création OrderData
                order_data = Confirm_Order(
                    ...
                    price_per_unit=round(total_price/total_quantity, 2),
                ...
                if not all([
                    ...
                    raise ValueError("Données manquantes pour la commande")
            ...
            return redirect(url_for('products.order_success'))

        except Exception as e:
            db.session.rollback()
            app.logger.error(f"ERREUR commande: {str(e)}", exc_info=True)
            flash(f"Échec de la confirmation: {str(e)}", 'danger')

    # Pré-remplissage (GET)
    elif request.method == 'GET':
        form.gender.data = form.gender.data or 'M'
        if session['shopcart']:
            prices = [float(p['price']) for p in session['shopcart'].values()]
            form.price_per_unit.data = round(sum(prices)/len(prices), 2)
