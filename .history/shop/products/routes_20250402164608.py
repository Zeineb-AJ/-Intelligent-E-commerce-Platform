...
@app.route('/')
def home():
    # Récupérer les produits en promotion (ex: les 3 premiers produits avec discount)
    promo_products = Product.query.filter(Product.discount > 0).order_by(Product.date_added.desc()).limit(3).all()
    
    # Récupérer les produits tendance (ex: les plus vendus)
    trend_products = Product.query.order_by(Product.sales.desc()).limit(4).all()
    
    # Pagination pour les produits normaux (votre code existant)
