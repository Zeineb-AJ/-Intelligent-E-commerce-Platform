...
@products.route('/confirm_order', methods=['GET', 'POST'])
@login_required
def confirm_order():
    if 'shopcart' not in session or not session['shopcart']:
        flash('Votre panier est vide !', 'warning')
        return redirect(url_for('products.cart'))  # Notez le namespace 'products.'
    ...
    if form.validate_on_submit():
        try:
            ...
            if form.photo.data:
                ...
                filepath = os.path.join('static/order_photos', filename)
                form.photo.data.save(filepath)
                analytics_data.photo_url = filepath
    ...
    return render_template('products/confirm.html',
                         ...
                         total=sum(float(p['price'])*int(p['quantity']) 
                              for p in session.get('shopcart', {}).values()))
